...
# READY
func _ready():
	# ✅ FIXED: force ruby to frame 0 of level2
...
func enemy_pick_card():
	var allowed = []
	for card in cards:
		if card.name == "Card1" or card.name == "Card3" or card.name == "Card4": #makes the fight go a bit smoother to use only the 3 cards
			allowed.append(card)
	if allowed.is_empty():
		return
...
func play_attack_animation(node):
	...
	if sprite == garnet_node:
		...
		sprite.play("level2")
		var frames = sprite.sprite_frames.get_frame_count("level2")
		var speed = sprite.speed_scale
		var fps = sprite.sprite_frames.get_animation_speed("level2")
		var duration = frames / (fps * speed)
		await get_tree().create_timer(duration).timeout
		sprite.stop()
		sprite.frame = 0
...
func heal(is_player):
	if is_player:
		...
		ruby_health += 1
		print("ruby healed 1 HP. HP = ", ruby_health)
		update_hearts()
		
		await play_heal_effect(ruby_heal_heart)
